...
import transfer_learning_basemodel_layeraddition
...
.st-fs
{
 background-color:none ;
    ...
    st.session_state['si_model_name']=None
    model_names =[];
    ...
    **Note:**
    ...
    """ 
        ...
        with st.form(key="magic"):
            ...
            #st.write(os.getcwd())
            if 'si_counter_magic' not in st.session_state:
                st.session_state['si_counter_magic']=0
            else:
                st.session_state['si_counter_magic']=0
            try:
            ...
            with col1:
                ...
                st.session_state['si_chkbox_csv_file']=Analyse_CSV.checkbox('Use CSV/Excel File For Transfer Learning',False)
                ...
                if uploaded_file is not None:
                  ...
                  st.session_state['si_user_data']=user_data
            ...
            with col3:
                 ...
                 st.session_state['si_data_external'] = st.text_input('GIT url To download csv File.(Click on View raw and Copy the URL)')
                 if len(model_names)>0:
                     st.session_state['si_model_name'] = st.selectbox(
            ...
            if flag:
                st.session_state['si_status_placeholder'].warning("Training Satrted...")
                transfer_learning_basemodel_layeraddition.app()
               
                
                            
        if 'si_status_placeholder' not in st.session_state:
            st.session_state['si_status_placeholder']=st.empty()
        if 'si_train_header' not in st.session_state:
              st.session_state['si_train_header']=st.empty() 
        
        if 'si_train_result' not in st.session_state:
             st.session_state['si_train_result']=st.empty() 
        if 'si_train_plot' not in st.session_state:
             st.session_state['si_train_plot']=st.empty() 
