...
import predict_base_model_tnsr
import os
import random
def app():
    ...
    if True or st.session_state['userid']!="":
        ...
        with st.form(key="magic"):
            ...
            if flag:
               ...
               predict_base_model_tnsr.app()
                
               
                            
        if 'tnsnr_status_predict_placeholder' not in st.session_state:
                 st.session_state['tnsnr_status_predict_placeholder']=st.empty() 
        
        if 'tnsnr_predict_result' not in st.session_state:
             st.session_state['tnsnr_predict_result']=st.empty() 
        if 'tnsnr_predict_header' not in st.session_state:
             st.session_state['tnsnr_predict_header']=st.empty() 
        if 'tnsnr_predict_df' not in st.session_state:
             st.session_state['tnsnr_predict_df']=st.empty()                   
    else:
