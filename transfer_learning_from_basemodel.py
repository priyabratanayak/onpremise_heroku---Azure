...
def app():   
    ...
    st.session_state['tl_status_placeholder'].warning("Fetching Customer Data...")
    
    # Loading data
    #tktdata = pd.read_excel(r'C:\Users\10689029\OneDrive - LTI\Documents\AI\ML\hackathon\code\knoweldge_transfer\Banner_health_3k.xlsx',sheet_name='Sheet1') #Sanjukta
    #tktdata = pd.read_excel(r'input/JCI_customer2.xlsx',sheet_name='Sheet1') #Sanjukta
    
    
    tktdata =None# pd.read_excel(r'input\JCI.xlsx')
    if st.session_state['tl_chkbox_csv_file']:
        tktdata=st.session_state['tl_user_data']
    else:
       tktdata=pd.read_excel('./input/Banner_customer2.xlsx') 
        
    if st.session_state['tl_data_external']:
        try:
            tktdata=pd.read_csv( st.session_state['tl_data_external'])
        except:
            tktdata=pd.read_excel('./input/Banner_customer2.xlsx') 
    st.session_state['tl_status_placeholder'].warning("Finished Fetching Customer Data...")
    ...
    st.session_state['tl_status_placeholder'].warning("Extracting Knowliedge from Base Model...")
    ...
    st.session_state['tl_status_placeholder'].warning("Loading base model...")
    # # Case-1 : - Only changing softmax/last layer in the pretrained layer
    
    # In[18]:
    
    
    #By selecting include_top=False, you get the pre-trained model without its final softmax layer so that you can add your own:
    #pretrained_model.trainable = False setting. It freezes the weights and biases of the pre-trained model so that you train your softmax layer only
    #However if you do have lots of data, transfer learning can work even better with pretrained_model.trainable = True.
    #The pre-trained weights then provide excellent initial values and can still be adjusted by the training to better fit your problem.
    ...
    layr=pre_trained_model.layers[3].output
    layr=Dense(NUM_CLASSES,activation='softmax')(layr)
    ...
    mc = ModelCheckpoint('./savedmodels/'+'best_new_model.hdf5', monitor='val_acc', mode='max', verbose=1, save_best_only=True)
    ...
    st.session_state['tl_status_placeholder'].warning("Transfer Learning completed...")
    # In[ ]:
    ## plotting
    ...
    st.session_state['tl_status_placeholder'].success("Training Finished.")
    st.session_state['tl_train_header'].subheader('Training Result')
    my_expander = st.session_state['tl_train_result'].expander(label='Training Summary',expanded=True)
    with my_expander:  
    ...
    st.session_state['tl_user_data']=None
    #==========Plot graph
    ...
    st.session_state['tl_status_placeholder'].warning("Transfered Model is ready...")
    ...
    st.session_state['tl_status_placeholder'].warning("Plotting Is In Progress...")
    new_model.save('saved_models/2')
    ...
    st.session_state['tl_train_plot'].image(new_image)
    st.session_state['tl_status_placeholder'].warning("Plotting Finished.")
    ...
    print(str(datetime.now())[:19]+'-> '+'Overall >90 CL percent::',round((tktdata[tktdata['Confidence_Probability'] >= 90].shape[0]/tktdata.shape[0])*100,2))
    print(str(datetime.now())[:19]+'-> '+'Overall >80 CL percent::',round((tktdata[tktdata['Confidence_Probability'] >= 80].shape[0]/tktdata.shape[0])*100,2))
    ...
    print(str(datetime.now())[:19]+'-> '+'Total time taken for prediction(Hr:Min:Sec) :',tm[:7])
    print(str(datetime.now())[:19]+'-> '+'Total number of rows processed              :',tktdata.shape[0])
    print('\n***********************************\n')
